...
(define (enqueue x q)
  (if (stream-empty? (queue-left q))
      (struct-copy queue q [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)] [right '()] [size-l (+ 1 (queue-size-r q))] [size-r 0] ) 
      (if (> (+ 1 (length (queue-right q))) (stream-length (queue-left q)) )
          (struct-copy queue q [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)] [right '()] [size-l (+ 1 (queue-size-l q) (queue-size-r q))] [size-r 0] )
...
(define (dequeue q)
  ( if (> (length (queue-right q)) (- (stream-length (queue-left q)) 1) )
         (struct-copy queue q [left (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)] [right '()] [size-l (- (+ (queue-size-l q) (queue-size-r q)) 1)] [size-r 0] )
         (if (stream-empty? (stream-rest (queue-left q)))
             (struct-copy queue q [left empty-stream] [right '()] [size-l  (queue-size-r q) ] [size-r 0] )
